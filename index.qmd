...
-   Before we jump into the motivating example, I want to briefly frame what we're doing and what I'm hoping you'll take away from my portion of the talk.
...
::: incremental
-   Explain why simple summary statistics alone can be misleading

-   Match a data type (continuous, categorical, or bivariate) to an appropriate figure

-   Recognize common pitfalls in the literature (e.g. dynamite plots, truncated axes, bar plots of means)

-   Understand when a box plot is not enough
:::

::: notes

-   By the end of my portion of the talk, I'm hoping you'll walk away with these four things.

-   First, an appreciation of why simple summary statistics alone - like a mean and standard deviation - can sometimes hide important features of your data.

-   Second, a rough decision guide for picking a figure based on the type of variable you're working with.

-   Third, an eye for a few common pitfalls that show up frequently in the published literature.

-   And finally, an understanding of when the tools we commonly reach for - like the box plot - are not quite enough on their own.
...
local({
  ...
  # Per-patient baselines and slopes with meaningful variability
  patient_effects <- bind_rows(
    tibble(
      patient = 1:n_per_arm,
      arm = "Treatment",
      baseline = rnorm(n_per_arm, 100, 10),
      slope = rnorm(n_per_arm, -4, 1.2)
    ),
    tibble(
      patient = (n_per_arm + 1):(2 * n_per_arm),
      arm = "Control",
      baseline = rnorm(n_per_arm, 100, 10),
      slope = rnorm(n_per_arm, 3.5, 1.2)
    )
  )
  ...
  data <- expand_grid(
    ...
    mutate(volume = baseline + slope * (week / 2) + rnorm(n(), 0, 7))
  ...
  p_line <- base_plot +
    geom_line(aes(group = patient), alpha = 0.7, linewidth = 0.6) +
  ...
  p_focus_treatment <- base_plot +
    geom_line(aes(group = patient), alpha = 0.8, linewidth = 0.6) +
...
* On the previous slide we introduced the scatter plot for two continuous variables. But when one of those variables is ordered - typically time - and we have repeated measurements per subject, a plain scatter plot can quickly become a cloud of points.
...
* This is why line plots are the default choice for repeated measures over time - biomarkers, tumor burden, patient-reported outcomes across follow-up visits, and so on.
